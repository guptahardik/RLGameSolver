...
for day in $(seq 0 $((num_days - 1))); do
    ...
    for ((i = 0; i < num_commits_today; i++)); do
        ...
        # Generate the commit date for the specified day
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # For macOS (using -v to adjust dates)
            commit_date=$(date -v-$day"d" -v$hour"H" -v$minute"M" -v$second"S" '+%Y-%m-%d %H:%M:%S')
        else
            # For Linux systems
...
# Uncomment the following lines to push to GitHub:
